...
async function startAPi(country,lang) {
  let responseAPI = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=d0988595ed434e01907155118241306%20&q=${country}&days=3&lang=${lang}`);
...
function displayWeatherCard1(currentWeather) {
  let display = ``;
  display = `<div class="weather-card h-100 ">
  ...
  display += `
    <div class="weather-card h-100 text-center">
            ...
            <div class="weather-card-body p-3">
                ...
                <div class="weather-main-degree">${Math.floor(currentWeather.forecast.forecastday[index].day.maxtemp_c)}<sup>o</sup>C</div>
                <div class="weather-degree mb-4">${Math.floor(currentWeather.forecast.forecastday[index].day.avgtemp_c)}<sup>o</sup>C</div>
...
function formatDate(dateString) {
  ...
  return {
    day: formattedDay, // returns the full name of the day 
    formattedDate: `${formattedDate}` // returns the formatted date 
  };
}
